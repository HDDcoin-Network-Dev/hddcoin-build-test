...
PATH=$(npm bin):$PATH
cd ../../ || exit 1
git submodule update --init hddcoin-blockchain-gui

cd ./hddcoin-blockchain-gui || exit 1
echo "npm build"
lerna clean -y
npm ci
...
# Other than `hddcoin-blockchain-gui/package/gui`, all other packages are no longer necessary after build.
